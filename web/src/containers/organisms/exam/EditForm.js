...
import {setQuestions, showModal, editExam} from "../../../redux/modules/Exam"
import {getExamCategories} from "../../../redux/modules/ExamCategory"
import * as validate from "../../../utils/Validate"
import {Button, Modal, ModalHeader, ModalBody, ModalFooter} from 'reactstrap'
import {getExamById, cleanSelectedExam, setEditStarted} from '../../../redux/modules/Exam'
import {getTodayStr} from "../../../utils/DateUtil"

class Edit extends Component {

  componentWillMount() {
    const {getExamById, getExamCategories} = this.props
    getExamCategories()
    let params = {
  ...
  componentWillUnmount() {
    ...
    this.props.cleanSelectedExam()
    this.props.setQuestions([{answers: [{}]}])
    this.props.setEditStarted(false)
  }
  ...
  handleSubmit(data) {
    ...
    editExam(data, this.props.match.params)
  }

  initValues() {
    const {selectedExam, setQuestions, setEditStarted, editStarted} = this.props
    if (selectedExam && selectedExam.questions && !editStarted) {
      setEditStarted(true)
              ...
              >
                <div className="card-block">
                  ...
                  <Field
                    ...
                    validate={[validate.required()]}
                  />
                  {questions && questions.map((question, qKey) => {
                    return (
                      <div className="row">
...
function mapStateToProps(state) {
  return {
    ...
    formProps: state.form.editExamForm,
    questions: state.exam.questions,
    isShow: state.exam.isShowCompleteModal,
    selectedExam: state.exam.selectedExam,
    me: state.auth.me,
    editStarted: state.exam.editStarted
  }
}

function mapDispatchToProps(dispatch) {
  return bindActionCreators(Object.assign({}, {
    getExamCategories, setQuestions, showModal,
    editExam, getExamById, cleanSelectedExam, setEditStarted
  }), dispatch)
}

let EditExamForm = reduxForm({
  form: 'editExamForm',
  enableReinitialize: true
})(Edit)

EditExamForm = connect(
  (state) => {
    return {
      initialValues: state.exam.selectedExam
    }
  }
)(EditExamForm)


export default withRouter(connect(mapStateToProps, mapDispatchToProps)(EditExamForm))